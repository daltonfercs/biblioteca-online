...
const EditBook = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errMsg || 'Error al modificar libro');
      }
      setSuccess(true);
      setTimeout(() => navigate('/catalogo'), 1500);
    } catch (err) {
      setError('No se pudo modificar el libro');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="edit-book-page" style={{ maxWidth: 500, margin: '2rem auto', padding: 24, background: '#fff', borderRadius: 8 }}>
      <h2>Editar Libro</h2>
      ...
      {success && <p style={{ color: 'green', marginTop: 8 }}>¡Libro modificado!</p>}
    </div>
  );
};
